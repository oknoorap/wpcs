...
class Sniffer extends EventEmitter {
  ...
  run() {
    ...
    if (!rulesetList.includes(this.ruleset)) {
      this.ruleset = `${rulesetTypes.WORDPRESS_EXTRA},${rulesetTypes.WORDPRESS_DOCS}`
    }
        ...
        const fixWorker = async.queue((filename, resolve) => {
          ...
          const waitInterval = setInterval(() => {
            if (waitingTime > 30) {
              ...
              phpcbf.kill('SIGKILL')
              this.process.emit('error', new Error(`Engine is too busy to fixing ${filename}, please do it manually.`))
            }
